...
export function UploadPlaybookDialog({ open, onOpenChange }: Props) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setLoading(true);
    const uploadPromise = playbooksApi.upload(form);
    toast.promise(uploadPromise, {
      loading: `Uploading playbook "${name}"…`,
      success: `Playbook "${name}" uploaded`,
    ...
    try {
      ...
      mutate(
        "/playbooks",
        (current: Playbook[] | undefined) => [...(current ?? []), created],
        false
      );
      reset();
      onOpenChange(false);
      setTimeout(() => mutate("/playbooks"), 2000);
    } catch {
  ...
  return (
    <Dialog open={open} onOpenChange={(v) => { if (!loading) { onOpenChange(v); if (!v) reset(); } }}>
      <DialogContent className="max-w-md border-surface-4 bg-surface-2 text-slate-700">
        <DialogHeader>
          <DialogTitle className="text-slate-900">Upload Playbook</DialogTitle>
        </DialogHeader>
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-1.5">
            ...
            <Input
              ...
              placeholder="Formats SQL queries in BigQuery style"
              className="border-surface-4 bg-surface-3 text-slate-900 placeholder:text-slate-500"
              disabled={loading}
            />
          </div>
          <div className="space-y-1.5">
            <Label className="text-xs text-slate-500">File (.zip) *</Label>
