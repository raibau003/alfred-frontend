...
// Dashboard financiero completo (todos los meses/bancos/productos/categorías + evolución).
export async function GET() {
  if (!SECRET) return NextResponse.json({ error: "sin ROUTER_ADMIN_SECRET" }, { status: 503 });
  try {
    const r = await fetch(`${ROUTER}/finance/dashboard`, { headers: { "x-admin-secret": SECRET }, cache: "no-store" });
